...
/*function encrypt(text) {
  ...
  // Return only the encrypted data (do not return the IV or full object to Mongoose)
  return encrypted;
}
...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Encrypt credit card info, CVV, and expiry date
  ...
  try {
    ...
    const userProfile = {
      ...
      creditCards: user.creditCardNumber.map((card, index) => ({
        cardLast4: card.slice(-4), // Extract last 4 digits of the card number
        expiryDate: user.expiryDate[index],
        cvv: user.cvv[index] // You can exclude this in the response if it's not needed
      })),
...
app.get('/user-profile', async (req, res) => {
  ...
  try {
    ...
    const userProfile = {
      ...
      // Decrypt credit card info, expiry date, and CVV
      creditCards: user.creditCardNumber.map((card, index) => ({
        cardNumber: decrypt(card.encryptedData, card.iv), // Decrypt the card number
        expiryDate: decrypt(user.expiryDate[index].encryptedData, user.expiryDate[index].iv), // Decrypt expiry date
        cvv: decrypt(user.cvv[index].encryptedData, user.cvv[index].iv) // Decrypt CVV
      })),
...
// Example of how to decrypt data in your GET route
/*app.get('/user-profile', async (req, res) => {
  const { email } = req.query;
...
/*app.post('/update-profile', async (req, res) => {
  const { email, name, password, billingInfo, creditCards, promotionOptIn } = req.body;

  try {
    console.log('Received update profile request for email:', email);
    let encryptedCards = [];

    // Check if credit cards are provided and encrypt them
    if (creditCards && creditCards.length > 0) {
      console.log('Encrypting provided credit card data...');
      encryptedCards = await Promise.all(
          creditCards.map(async (card) => {
            // Validate the credit card data using the updated field names
            if (!card.creditCardNumber || !card.expiryDate || !card.cvv) {
              console.error('Missing required credit card fields:', card);
              throw new Error('Incomplete credit card data');
            }
            console.log('Encrypting card:', card.creditCardNumber);
            // Encrypt the card number using the provided encrypt function
            const encryptedCard = encrypt(card.creditCardNumber); // Encrypt only the card number here
            console.log('Encrypted card:', encryptedCard);
            return {
              encryptedCardData: encryptedCard.encryptedData,
              iv: encryptedCard.iv,  // Store the IV for decryption later
              expiryDate: card.expiryDate,
              cvv: card.cvv,         // You can decide if you want to encrypt these as well
            };
          })
      );
    } else {
    ...
    if (creditCards && creditCards.length > 0) {
      ...
      encryptedCards = await Promise.all(
          creditCards.map(async (card) => {
            ...
            // Encrypt card number and cvv
            const encryptedCardNumber = encrypt(card.creditCardNumber);
            const encryptedCvv = encrypt(card.cvv);

            return {
              encryptedCardData: encryptedCardNumber.encryptedData,
              cardIv: encryptedCardNumber.iv,
              expiryDate: card.expiryDate,
              encryptedCvvData: encryptedCvv.encryptedData,  // Optionally encrypt CVV
              cvvIv: encryptedCvv.iv,
            };
          })
      );
